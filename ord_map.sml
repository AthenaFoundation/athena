functor makeOrdMap(Key : ORD_KEY) :> ORDERED_MAP 
	where type ord_key = Key.ord_key = 

  struct

   type ord_key = Key.ord_key
...
in
    ...
    fun insert (E,x,v) = T{key=x,value=v,cnt=1,left=E,right=E}
      | insert (T(set as {key,left,right,value,...}),x,v) =
          if compare(key,x) = General.GREATER then T'(key,value,insert(left,x,v),right)
          else if compare(key,x) = General.LESS then T'(key,value,left,insert(right,x,v))
